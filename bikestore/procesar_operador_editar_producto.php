...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errores)) {
        ...
        try {
            // Actualizar producto con la primera talla (compatibilidad)
            $primera_talla = array_key_first($tallas_con_stock);
            
            $sql = "UPDATE productos SET nombre = ?, tipo = ?, talla = ?, nivel_ciclismo = ?, peso = ?, velocidades = ?, precio = ?, imagen = ?, imagen_principal = ?, imagen_2 = ?, imagen_3 = ?, imagen_4 = ?, imagen_5 = ?, imagen_6 = ?, stock = ?, dias_envio = ?, descripcion = ? WHERE id = ?";
            ...
            $stmt->bind_param("sssssissssssssiisi", 
                $nombre,           
                $tipo,             
                $primera_talla,    // Primera talla
                $nivel_ciclismo,   
                $peso,             
                $velocidades,      
                $precio,           
            ...
            // PASO 1: Desactivar todas las tallas anteriores (en lugar de eliminarlas)
            $stmt_desactivar = $conn->prepare("UPDATE producto_tallas SET activo = 0 WHERE producto_id = ?");
            $stmt_desactivar->bind_param("i", $producto_id);
            
            if (!$stmt_desactivar->execute()) {
                throw new Exception("Error al desactivar tallas: " . $stmt_desactivar->error);
            }
            $stmt_desactivar->close();
            
            // PASO 2: Insertar o actualizar las tallas seleccionadas
            $stmt_talla = $conn->prepare("
                INSERT INTO producto_tallas (producto_id, talla, stock, activo) 
                VALUES (?, ?, ?, 1)
            ...
            foreach ($tallas_con_stock as $talla => $stock) {
                ...
                if (!$stmt_talla->execute()) {
                    throw new Exception("Error al actualizar talla $talla: " . $stmt_talla->error);
                }
            }
            ...
            header("Location: principal_operador.php?success=" . urlencode("Producto '$nombre' actualizado exitosamente con $mensaje_tallas"));
            exit();
    ...
    if (!empty($errores)) {
        ...
        header("Location: operador_editar_producto.php?id=$producto_id&error=" . urlencode($mensaje_error));
        exit();
    }
